...
double *interp2dpol( double **grid, double *g_rad, double *g_ang, \
    ...
    int i, j;
    static double *interang, *result;
    static gsl_spline **splines;
    static gsl_interp_accel **accs;
    gsl_spline  *rspline;
    ...
    // allocate memory
    if (first_time){
      accs = (gsl_interp_accel **) malloc( n_rad * sizeof( gsl_interp_accel * ) );   //rjl - changed to use sizeof(pointer)  
      result = (double *) malloc( n_int * sizeof( double ) );    
      interang = (double *) malloc( n_rad * sizeof( double ) );
      splines = (gsl_spline **) malloc( n_rad * sizeof( gsl_spline * ) );            //rjl - changed to use sizeof(pointer)
      first_time=0;
    }
